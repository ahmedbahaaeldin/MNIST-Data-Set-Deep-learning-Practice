...
def train_Neural_network(x):
    ...
    with tf.Session() as ses:
        ...
        for epic in range(howmanyepics):
            epic_loss=0
            for _ in range(int(mnist.train.num_examples/batch_size)):
                epicx,epicy=mnist.train.next_batch(batch_size)
                _,c=ses.run([optimizer,cost] , feed_dict={x: epicx,y: epicy})
                epic_loss+=c
            print('Epic ',epic+1,' completed out of ',howmanyepics,'loss :',epic_loss)
